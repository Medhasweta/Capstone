...
class KGEModel(torch.nn.Module):
    ...
    """
    def __init__(self, num_nodes: int, num_relations: int, hidden_channels: int, sparse: bool = False, num_negatives: int = 1):
        super().__init__()
    ...
    @torch.no_grad()
    def random_sample(self, head_index: Tensor, rel_type: Tensor, tail_index: Tensor) -> Tuple[
        Tensor, Tensor, Tensor]:
        """Generates negative samples for a batch of positive triplets."""
        batch_size = head_index.size(0)
        total_samples = batch_size * (self.num_negatives + 1)  # +1 for positive samples

        # Randomly choose whether to replace heads or tails for negatives
        replace_heads = torch.randint(0, 2, (total_samples,))
        replace_indices = torch.randint(0, self.num_nodes, (total_samples,), device=head_index.device)

        neg_head_index = head_index.repeat((self.num_negatives + 1,)).clone()
        neg_tail_index = tail_index.repeat((self.num_negatives + 1,)).clone()
        neg_rel_type = rel_type.repeat((self.num_negatives + 1,))

        # Replace heads or tails with random indices for negatives
        neg_head_index[replace_heads.bool()] = replace_indices[replace_heads.bool()]
        neg_tail_index[~replace_heads.bool()] = replace_indices[~replace_heads.bool()]

        # Exclude the first `batch_size` samples as they are positives
        return neg_head_index[batch_size:], neg_rel_type[batch_size:], neg_tail_index[batch_size:]
