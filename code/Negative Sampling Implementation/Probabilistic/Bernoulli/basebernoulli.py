...
class KGEModel(torch.nn.Module):
    ...
    def random_sample(
        ...
        probs = torch.rand(head_index.size(), device=head_index.device)
        mask = torch.bernoulli(probs).long()  # Ensure mask is long type

        # Improved logic for replacing heads/tails (avoid in-place operations)
        head_or_tail = torch.bernoulli(torch.ones(num_negatives, device=head_index.device)).long()
        rnd_index = torch.randint(self.num_nodes, head_index.size(), device=head_index.device)

        new_head_index = head_index.clone()  # Create a copy to avoid in-place modification
        new_tail_index = tail_index.clone()

        if head_or_tail.sum() > 0:  # Replace heads if at least one head is chosen
            new_head_index[mask] = rnd_index[mask]
        else:  # Replace tails if no heads are chosen
            new_tail_index[~mask] = rnd_index[~mask]
