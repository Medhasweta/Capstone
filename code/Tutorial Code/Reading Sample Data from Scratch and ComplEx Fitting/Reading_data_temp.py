...
# os.chdir('/Users/medhaswetasen/Documents/GitHub/Capstone/code/Tutorial Code')
class TempDataset(torch_geometric.data.InMemoryDataset):
    ...
    def process(self):
        with open(os.path.join(self.raw_dir, 'entities.dict'), 'r') as f:
            ...
            entities_dict = {key: int(value) for value, key in lines if len(key) > 0 and len(value) > 0}

        with open(os.path.join(self.raw_dir, 'relations.dict'), 'r') as f:
            lines = [row.split('\t') for row in f.read().split('\n') if row]  # Add `if row` to filter out empty strings
            relations_dict = {key: int(value) for value, key in lines if len(key) > 0 and len(value) > 0}
...
Temp_dset = TempDataset(root='Temp Data')
data = Temp_dset[0]
...
# print(data.train_edge_type.size()[0])
# print(data.train_edge_type.size()[-1])
